...
function isVow(a) {
  return a.map((el) =>
    String.fromCharCode(el).match(/[aeiou]/gi) ? String.fromCharCode(el) : el
  );

  //   return a.map((el) =>
  //     /[aeiou]/gi.test(String.fromCharCode(el)) ? String.fromCharCode(el) : el
  //   );

  // return a.map(el => 'aeiou'.includes(res = String.fromCharCode(el)) ? res : el)
}
